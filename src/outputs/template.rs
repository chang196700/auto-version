...
pub fn _render_simple(info: &VersionInfo, template: &str) -> Result<String> {
    let vars: &[(&str, String)] = &[
        ...
        ("build_metadata",         info.build_metadata.clone().unwrap_or_default()),
        ...
        ("branch_name_slug",       info.branch_name_slug.clone().unwrap_or_default()),
        ("sha",                    info.sha.clone().unwrap_or_default()),
        ("short_sha",              info.short_sha.clone().unwrap_or_default()),
        ("commits_since_tag",      info.commits_since_tag.map(|v| v.to_string()).unwrap_or_default()),
        ("uncommitted_changes",    info.uncommitted_changes.map(|v| v.to_string()).unwrap_or_default()),
